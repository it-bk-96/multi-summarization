...
if __name__ == '__main__':
    ...
    textRank = TextRank()
    total_summary = []
    doc_folders = root_directory + "Data/VietNamese/Documents"
    ...
    for folder in os.listdir(doc_folders):
        path = doc_folders + "/" + folder
        print("Running TextRank_results Summarizer for files in folder: ", folder)
        doc_summary = []
        ...
        length_summary = int((len(text_1_token.split()) + len(text_2_token.split())) / 2)

        summary = textRank.main(length_summary, path)
        for senten in summary:
            text_append = re.sub("\n", "", senten.getOGwords())
            text_append = text_append + " "
            doc_summary.append(text_append)

        results_folder = root_directory + "Data/VietNamese/TextRank_results"
