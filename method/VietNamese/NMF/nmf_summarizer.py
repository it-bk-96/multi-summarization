...
def makeSummary_Gong_Liu(sentences, n, H):
    ...
    current_length = len(ViTokenizer.tokenize(sentences[index_H].getOriginalWords().strip()).split())
    array_index = [index_H]
...
if __name__ == '__main__':
    ...
    for folder in os.listdir(main_folder_path):
        ...
        # summary = makeSummary_Topic(sentences, length_summary, H)đó

        final_summary = ""
        for sent in summary:
            final_summary = final_summary + sent.getOriginalWords() + "\n"
        final_summary = final_summary[:-1]
        results_folder = root_directory + "Data/VietNamese/NMF_results"
