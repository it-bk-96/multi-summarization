...
from method.English.SVM.convert import text_utils_english
from method.English.SVM.convert import pyrouge_convert

DIR_PATH = ROOT_DIR + '/Data/DUC_2007'
    ...
    def convert_extract(self, list_documents, list_human, path_save):
        ...
        while (0 == 0):
            ...
            for sent in all_sentences:
                ...
                tmp += sent
                # Use rouge 1
                tmp_rouge_f1 = pyrouge_convert.rouge_1(tmp, sentences_reference, self.alpha)
...
if __name__ == "__main__":
    ...
    list_clusters = []
    list_human_refs = []
    dir_documents = DIR_PATH + '/Documents_process'
    for clus in os.listdir(dir_documents):
        list_clusters.append(dir_documents + '/' + clus)
        id_cluster = clus.split("D07").pop()[:-1]
        list_human_refs.append([
                DIR_PATH + '/Human_Summaries/' + 'summary_' + str(id_cluster) + '.A.1.txt',
                DIR_PATH + '/Human_Summaries/' + 'summary_' + str(id_cluster) + '.B.1.txt',
                DIR_PATH + '/Human_Summaries/' + 'summary_' + str(id_cluster) + '.C.1.txt',
                DIR_PATH + '/Human_Summaries/' + 'summary_' + str(id_cluster) + '.D.1.txt'
        ])
    rouges = []
    for i in range(len(list_clusters)):
        ...
        rouges.append(convert_extract.convert_extract(list_docs_in_clus, list_human_ref,
                                                      ROOT_DIR + "/method/English/SVM/Data/converted_standard/" + name_clus))
