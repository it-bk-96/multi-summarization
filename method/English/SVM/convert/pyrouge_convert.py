...
def rouge_1(peer, models, alpha):
    ...
    models.
    """
    peer = nltk.word_tokenize(peer)

    peer = list(filter(lambda x: x != '.' and x != '`' and x != ',' and x != '_' and x != ';'
    ...
    model_4 = []
    for model in models:
        model = nltk.word_tokenize(model)

        model = list(filter(lambda x: x != '.' and x != '`' and x != ',' and x != '_' and x != ';'
                                     and x != '(' and x != ')' and x.find('&') == -1
                                     and x != '?' and x != "'" and x != '!' and x != '''"'''
                                     and x != '``' and x != '--' and x != ':'
                                     and x != "''" and x != "'s", model))
        model_4.append(model)
    return rouge_n(peer, model_4, 1, alpha)
