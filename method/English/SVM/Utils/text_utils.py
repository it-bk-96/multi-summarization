...
def text_process_all(sentences):
    new_sentences = []
...
def get_sentence_first_paragraph(file_name):
    with open(file_name, 'r') as file:
        text = file.readlines()
...
def read_all_documents(file_names):
    documents = []
    for item in file_names:
        sentences = split_sent_json(item)
        sentences = text_process_all(sentences)  # remove stopwords
