...
def cos_similarity(idf_dict, doc):
    '''
    compute cosine similarity of any sentence with first sentence of once document
    :param s1:
    :param s2:
    ...
    list_sents = doc.split('\n')
    first_sent = list_sents[0]

    all_words = set(doc.replace('\n', ' ').split(' '))

    tf_arr = {}
    for word in all_words:
        tf_arr[word] = tf(word, doc)

    list_words_first_sent = first_sent.split(' ')
    for word in list_words_first_sent:
        denom1 += (tf_arr[word] * idf_dict[word]) ** 2

    similarities.append(first_sent, 1.0)
    for sent in list_sents[:-1]:
        for word in sent.split(' '):
            denom2 += (tf_arr[word] * idf_dict[word]) ** 2
            if word in list_words_first_sent:
                numerator += (tf_arr[word] * idf_dict[word]) ** 2

        similarities.append((sent, numerator / (math.sqrt(denom1) * math.sqrt(denom2))))
