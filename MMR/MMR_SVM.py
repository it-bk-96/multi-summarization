...
def sentence_sim(sentence1, sentence2, idf, doc):
    ...
    for word in sentence2["value"].split(" "):
        numerator += (text_utils.tf(word, doc[sentence1["doc"]]) * idf[word]) ** 2

    for word in sentence1["value"].split(" "):
        denominator += text_utils.tf(word, doc[sentence1["doc"]]) * text_utils.tf(word, doc[sentence2["doc"]]) * idf[word]**2
